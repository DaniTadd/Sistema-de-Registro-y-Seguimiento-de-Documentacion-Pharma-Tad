function main(
  ...
  try {
    ...
    // Prioridad absoluta a los datos que vienen del panel de Power Automate
    ...
    if (!searchId || searchId === "") {
      ...
      actionResult.message = `Se necesita un ID de ${ENT} en el panel para realizar esta acción.`;
      actionResult.logLevel = 'WARN';
    } else if (!motivo || motivo === "") {
      actionResult.success = false;
      actionResult.message = `El motivo es obligatorio en el panel para cambiar el estado de ${ART} ${ENT}.`;
      actionResult.logLevel = 'WARN';
    } else {
      ...
      if (!found) {
        ...
        if (currentState === "ANULADO") {
          ...
          // Limpiamos el formulario (usando las etiquetas detectadas en labelRange)
          if (labelRange) {
