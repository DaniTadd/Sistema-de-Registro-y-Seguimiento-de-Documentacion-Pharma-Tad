 ...
 * OBJETIVO: Gestionar el cierre y reapertura de registros (Toggle) con bloqueo por dependencias y validación criptográfica.
 */
async function main(workbook: ExcelScript.Workbook, idConfirmacion: string, motivoDeCambio: string, claveFirma: string) {
  ...
  if (!configuracionActiva) throw new Error(`La hoja '${nombreHojaActiva}' no es un formulario válido para transición de estado.`);
  ...
  try {
    ...
    if (rangoEtiquetasFormulario) {
      ...
      // --- 2. VALIDACIONES PREVIAS DE TRANSICIÓN ---
      if (!idABuscar || idABuscar === "") {
        listaErroresValidacion.push(`Se requiere un ID de ${etiquetaEntidad} válido.`);
      }
      ...
      if (!motivoDeCambio || motivoDeCambio.trim() === "") {
        listaErroresValidacion.push(`La justificación es obligatoria (ALCOA+).`);
      }

      // Procedemos a buscar solo si la firma y los datos básicos están correctos
      if (listaErroresValidacion.length === 0) {
        ...
        if (!registroEncontrado) {
          ...
          // Inmutabilidad de Anulados
          if (estadoActual === "ANULADO") {
            listaErroresValidacion.push(`Un registro ANULADO es definitivo y no permite cambios de estado.`);
          } else {
            const nuevoEstado: string = (estadoActual === "ABIERTO") ? "CERRADO" : "ABIERTO";

            // --- 3. REGLAS DE NEGOCIO: DEPENDENCIAS (Cierre Condicionado) ---
                ...
                tablaReglasMaestra.getRangeBetweenHeaderAndTotal().getValues().forEach((regla: ValorCelda[]) => {
                  if (nombreTablaPrincipal.toUpperCase().includes(String(regla[0]).toUpperCase())) {
                    ...
                    if (operador === "DEPENDENCIAS_CERRADAS") {
                      const referenciaRaw: string = String(regla[3]); // Ej: TablaCapas[ID_DESVIO]
                      const mensajeErrorRegla: string = String(regla[4]);
                      
                      if (referenciaRaw.includes("[")) {
                        ...
                        if (tablaReferencia) {
                          const matrizReferencia: ValorCelda[][] = tablaReferencia.getRangeBetweenHeaderAndTotal().getValues() as ValorCelda[][];
                          const encabezadosRef: string[] = tablaReferencia.getHeaderRowRange().getValues()[0].map((h: ValorCelda) => String(h).toUpperCase().replace(/\s/g, "_"));
                          const idxForanea: number = encabezadosRef.indexOf(nombreColumnaRef);
                          const idxEstadoRef: number = encabezadosRef.indexOf("ESTADO");

                          if (idxForanea !== -1 && idxEstadoRef !== -1) {
                            const tieneDependenciaAbierta: boolean = matrizReferencia.some((filaRef: ValorCelda[]) => {
                              return String(filaRef[idxForanea]).toUpperCase() === idABuscar && String(filaRef[idxEstadoRef]).toUpperCase() === "ABIERTO";
                            });
    ...
    if (resultadoOperacion.message === "Inicio de transición") resultadoOperacion.message = `❌ Error Crítico: ${String(e)}`;
  } finally {
