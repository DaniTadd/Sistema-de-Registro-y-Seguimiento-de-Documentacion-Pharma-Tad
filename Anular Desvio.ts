function main(workbook: ExcelScript.Workbook) {
  ...
  const UX = {
    ...
    WARN_BG: "#FFF3CD", WARN_TXT: "#856404"
  };

  let sePuedeProcesar = true;
  let mensajeSalida = "Inicio de anulación.";
  ...
  if (sePuedeProcesar) {
    try {
      ...
      if (usuarioVal === "") errores.push("Firma de Usuario obligatoria para anular.");
      if (motivoVal === "") errores.push("Motivo obligatorio para anular.");
      ...
      if (rowIndex === -1 && idBuscado) errores.push(`ID ${idBuscado} no encontrado.`);
      if (rowIndex !== -1 && estadoActual.toUpperCase() === "ANULADO") {
          errores.push("El registro YA está anulado.");
      }
      ...
      if (errores.length > 0) {
          ...
          tablaBD.getRangeBetweenHeaderAndTotal().getCell(rowIndex, idxEstado).setValue("ANULADO");
          ...
          const rowHistData: (string | number | boolean)[] = headersHist.map((h: string) => {
             ...
             if (key === "CAMBIOS") return `[ANULACIÓN: Estado ${estadoActual} -> ANULADO]`;
             return "";
          });

          tHist.addRow(-1, rowHistData);

          // 3. Limpieza y Mensaje
          ...
          msj.setValue(`⚠️ Desvío ${idBuscado} ha sido ANULADO permanentemente.`);
          msj.getFormat().getFill().setColor(UX.WARN_BG);
          msj.getFormat().getFont().setColor(UX.WARN_TXT);
