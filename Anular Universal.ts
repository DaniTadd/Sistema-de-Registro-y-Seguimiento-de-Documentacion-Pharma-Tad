function main(
  ...
  try {
    ...
    if (labelRange) {
      ...
      if (!searchId || searchId === "N/A") {
        ...
        actionResult.message = `Se necesita un ID de ${ENT} para anular.`;
        actionResult.logLevel = 'WARN';
      } else if (!motivo || motivo === "N/A" || String(motivo).trim() === "") {
        actionResult.success = false;
        actionResult.message = `No se puede anular, el motivo es obligatorio.`;
        actionResult.logLevel = 'WARN';
      } else {
        ...
        if (!found) {
          ...
          if (estIdx !== -1) dbTab.getRangeBetweenHeaderAndTotal().getCell(rIdx, estIdx).setValue("ANULADO");
  ...
  function clearForm(ws: ExcelScript.Worksheet, ls: CellValue[][], o: number, pk: string) {
    ls.forEach((row, i) => {
      ...
      // Limpiamos todo excepto el ID y el Usuario (que suele ser el logueado)
      if (c !== "" && c !== pk && c !== "USUARIO") {
